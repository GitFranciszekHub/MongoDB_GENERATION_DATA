...
def wyszukiwanie_wartosci():
        ...
        if i == zmienna_id:
            try:
                    ...
                    tworzenieGrafu(ostatnia_czestosc_oddechu, ostatnia_SaturacjaTlenem, ostatnia_Puls, ostatnia_temperatura_ciala, 
                                ...
                                ,ostatnia_Hemoglobina,ostatnia_Erytrocyty,ostatnia_Leukocyty,ostatnia_Plytki_krwi,ostatnia_cisnienie)
                    break
    ...
    if komunikat == False:
        print("Nie znaleziono paujących tajnych ID.")
    time.sleep(1)
...
def tworzenieGrafu(ostatnia_czestosc_oddechu, ostatnia_SaturacjaTlenem, ostatnia_Puls, ostatnia_temperatura_ciala, ostatnia_poziom_potasu, sod, ostatnia_konsumpcja_tlenu
    ...
    ListaSplitCisinieine = ostatnia_cisnienie.split("/")
    pierwszeCisinienie = int(ListaSplitCisinieine[0])
    drugieCisinienie = int(ListaSplitCisinieine[1])
    ...
    lista_wartosci = [
        ...
        lista_tworzenie_cisnienie_pierwszeCisinienie,
        lista_tworzenie_cisnienie_drugieCisinienie
    ]
    ...
    ax.plot(lista_tworzenie_cisnienie_pierwszeCisinienie, label="ciśnienie skurczowe")
    ax.plot(lista_tworzenie_cisnienie_drugieCisinienie, label="ciśnienie rozkurczowe")

    ax.set_xlim(0, 5)
    ax.set_ylim(70, 140)  
    ax.set_title("Ciśnienie")
    ax.legend()  
    plt.savefig('static/cisnienie.png')
    plt.close(fig)
